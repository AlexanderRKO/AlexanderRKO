...
| ------- | ------------ | --- |
| Windows launcher says `ERROR: Python 3.10 or later not found` but you've installed Python | Python was installed without "Add to PATH" ticked, OR Explorer cached its PATH before the install completed | Pull the latest `Start Migration Dashboard.bat` — it now scans default install locations (`%LOCALAPPDATA%\Programs\Python\…`, `%ProgramFiles%\Python*\…`) and finds Python even when PATH is broken. If still failing: re-run the python.org installer → **Modify** → tick both *Add python.exe to PATH* and *py launcher*, then **log out and back in** (Explorer caches PATH at startup; new processes won't see updates until it restarts) |
| Windows launcher silently opens the Microsoft Store | PATH is pointing at the Microsoft Store `python.exe` stub | Don't use Microsoft Store Python — install from https://www.python.org/downloads/ instead. The launcher's `import sys` health-check now rejects the Store stub |
